...
def dashboard_summary(
    ...
    total_sales = money(db.query(func.coalesce(func.sum(SalesInvoice.grand_total), 0)).filter(
        SalesInvoice.company_id == company_id
    ).scalar())
    today_sales = money(db.query(func.coalesce(func.sum(SalesInvoice.grand_total), 0)).filter(
        SalesInvoice.company_id == company_id,
        SalesInvoice.invoice_date == today,
    ).scalar())
    monthly_sales = money(db.query(func.coalesce(func.sum(SalesInvoice.grand_total), 0)).filter(
        SalesInvoice.company_id == company_id,
        SalesInvoice.invoice_date >= month_start,
        SalesInvoice.invoice_date <= today,
    ).scalar())

    today_purchases = money(db.query(func.coalesce(func.sum(PurchaseInvoice.grand_total), 0)).filter(
        PurchaseInvoice.company_id == company_id,
        PurchaseInvoice.purchase_date == today,
    ).scalar())
    monthly_purchases = money(db.query(func.coalesce(func.sum(PurchaseInvoice.grand_total), 0)).filter(
        PurchaseInvoice.company_id == company_id,
        PurchaseInvoice.purchase_date >= month_start,
        PurchaseInvoice.purchase_date <= today,
    ).scalar())
    today_collections = money(db.query(func.coalesce(func.sum(Payment.amount), 0)).filter(
        Payment.company_id == company_id,
        Payment.payment_date == today,
    ).scalar())

    payment_total = money(db.query(func.coalesce(func.sum(Payment.amount), 0)).filter(
        Payment.company_id == company_id
    ).scalar())
    opening_total = money(db.query(func.coalesce(func.sum(Customer.opening_balance), 0)).filter(
        Customer.company_id == company_id
    ).scalar())
    total_receivables = money(opening_total + total_sales - payment_total)

    stock_rows = (
        db.query(CurrentStock, Batch)
        .join(Batch, Batch.id == CurrentStock.batch_id)
        .filter(CurrentStock.company_id == company_id)
        .all()
    )
    ...
    product_stock = (
        db.query(
            ...
            func.coalesce(func.sum(CurrentStock.quantity_available), 0).label("available"),
        )
    ...
    today_profit = db.query(
        func.coalesce(func.sum(SalesInvoiceItem.taxable_amount - (SalesInvoiceItem.quantity * Batch.purchase_rate)), 0)
    ).join(
        SalesInvoice, SalesInvoice.id == SalesInvoiceItem.sales_invoice_id
    ).join(
        Batch, Batch.id == SalesInvoiceItem.batch_id
    ).filter(
        SalesInvoice.company_id == company_id,
        SalesInvoice.invoice_date == today,
    ).scalar()
